 ...
 */
metadata {
	definition (name: "Z-Wave August Lock OLD", namespace: "me.bendy.augustOLD", author: "Ben Rimmasch", minHubCoreVersion: '000.017.0012', executeCommandsLocally: false) {
		capability "Actuator"
		capability "Lock"
		capability "Polling"
		capability "Refresh"
        ...
        // August
        fingerprint mfr:"033F", prod:"0001", model:"0001", deviceJoinName: "August Third Generation Z-Wave Door Lock"
		// Generic
	...
	tiles(scale: 2) {
        ...
        standardTile("door_state", "device.door_state", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
            state "lock_close", label:"locked/closed", icon:"st.contact.contact.closed", backgroundColor:"#dc1500"
            state "open", label:"open", icon:"st.contact.contact.open", backgroundColor:"#dccd00"
            state "close", label:"closed", icon:"st.contact.contact.closed", backgroundColor:"#54dc00"
        }
		...
		main "toggle"
		details(["toggle", "lock", "unlock", "battery", "door_state","refresh"])
	}
}
...
def zwaveEvent(DoorLockOperationReport cmd) {
    ...
    def map_door = [ name: "door_state" ]
	map_door.data = [ lockName: device.displayName ]
    def contact_state
    if (cmd.doorCondition == 0x01) {
        map_door.value = "lock_close"
        map_door.descriptionText = "Locked/Closed"
        contact_state = "closed"
    }
    else if (cmd.doorCondition == 0x02) {
        map_door.value = "open"
        map_door.descriptionText = "Open"
        contact_state = "open"
    }
    else if (cmd.doorCondition == 0x03) {
        map_door.value = "close"
        map_door.descriptionText = "Closed"
        contact_state = "closed"
    }

    def contactEvent = createEvent(name: "contact", value: contact_state)
	...
	if (generatesDoorLockOperationReportBeforeAlarmReport()) {
        ...
        runIn(3, "delayLockEvent", [data: [map: map_door]])
        return contactEvent
	} else {
        def events = [createEvent(map), createEvent(map_door), contactEvent]
        //events << createEvent(map_door)
        //log.debug "first ${events}"
        //events += contactEvent
        //log.debug "second ${events}"
        //log.debug "events ${events}"
        //todo: figure out why this method now fires twice
